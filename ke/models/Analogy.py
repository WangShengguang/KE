...
class Analogy(Model):
    ...
    def forward(self):
        ...
        res_comp = -tf.reduce_sum(
            self._calc_comp(h_embed_1, h_embed_2, t_embed_1, t_embed_2, r_embed_1, r_embed_2), 1,
            keep_dims=False)
        ...
        loss_func = tf.reduce_mean(tf.nn.softplus(tf.reshape(self.input_y, [-1]) * tf.reshape(self.predict, [-1])),
                                   0, keep_dims=False)
        regul_func = tf.reduce_mean(h_embed_1 ** 2) + tf.reduce_mean(t_embed_1 ** 2) + tf.reduce_mean(
            h_embed_2 ** 2) + tf.reduce_mean(
            t_embed_2 ** 2) + tf.reduce_mean(r_embed_1 ** 2) + tf.reduce_mean(r_embed_2 ** 2) + tf.reduce_mean(
            h_embed ** 2) + tf.reduce_mean(
            t_embed ** 2) + tf.reduce_mean(r_embed ** 2)
