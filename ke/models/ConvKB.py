...
class ConvKB(Model):
    def __init__(self, data_set, num_ent_tags, num_rel_tags, ent_emb_dim, rel_emb_dim):
        super().__init__(data_set, num_ent_tags, num_rel_tags, ent_emb_dim, rel_emb_dim)
    ...
    def embedding_def(self, num_ent_tags, num_rel_tags, ent_emb_dim, rel_emb_dim):
        # Embedding layer
        self.W = tf.Variable(tf.random_uniform([num_ent_tags + num_rel_tags, self.embedding_size],
                                               -math.sqrt(1.0 / self.embedding_size),
                                               math.sqrt(1.0 / self.embedding_size), seed=1234), name="W")
    ...
    def forward(self):
        ...
        total_dims = (self.embedding_size * len(self.filter_sizes) - sum(self.filter_sizes) + len(
